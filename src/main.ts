...
async function run(): Promise<void> {
  ...
  try {
    ...
    if (error?.message?.startsWidth('No commits between')) {
      core.info(
        `No commits between ${context.repo.owner}:${base} and ` +
          `${owner}:${head}`
      )
    } else if (
      error?.message?.startsWidth('A pull request already exists for')
    ) {
      core.info(
        `A PR already open from ${context.repo.owner}:${base} to ` +
          `${owner}:${head}`
      )
    } else {
